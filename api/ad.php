...
<?php
  ...
  */
/*
    ini_set('include_path', 'lib/PEAR/XML/');
   
    require 'openads-xmlrpc.inc.php';

    if (!isset($OA_context)) $OA_context = array();

    $oaXmlRpc = new OA_XmlRpc('ads.notfix.de', '/www/delivery/axmlrpc.php', 80, false, 15);

    //view($what='', $campaignid=0, $target='', $source='', $withText=false, $context=array(), $charset='')
    $adArray = $oaXmlRpc->view('zone:2', 0, '', '', 0, $OA_context, '');
  ...
  echo $adArray['html'];
	...
	*/
	...
	require 'openads-xmlrpc.inc.php';
  $oaXmlRpc = new OA_XmlRpc('ads.notfix.de', '/www/delivery/axmlrpc.php', 0, false, 2);
 
 $adArray = $oaXmlRpc->spc(array('XXL'=>2, 'FSB'=>488, 'SKY'=>489, 'MR'=>490, 'LAYER'=>491), '', 'wwe_0055_a_0001_b_0001_c_', 0, 1, 1, '');
